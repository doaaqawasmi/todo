<template>
  <div class="team">
    <h1 class="body-2 grey--text">Team</h1>

    <v-container class="my-15"> <!--ممكن احط fluid-->
      <v-layout wrap>
        <v-flex xs12 sm6 md4 lg3 v-for="person in team" :key="person.name">
          <v-card flat class="text-center ma-3">

            <v-responsive class="pt-4"> <!--pt -> padding top-->
              <v-avatar size="100" class="grey lighten-4">
                <img :src="person.avatar">
              </v-avatar>
            </v-responsive>

            <v-card-text>
              <div class="text-subtitle-2">{{ person.name }}</div>
              <div class="grey--text">{{ person.role }}</div>
            </v-card-text>

            <v-card-actions>
              <v-btn text color="grey">
                <v-icon small left>mdi-message-text</v-icon>
                <span>Message</span>
              </v-btn>
            </v-card-actions>

          </v-card>
        </v-flex>
      </v-layout>
    </v-container>

  </div>
</template>

<script>
export default {
  data() {
    return {
      team: [
        { name: 'Yoshi', role: 'Sales guru', avatar: '/avatar-5.png' },
        { name: 'Chun Li', role: 'web daveloper', avatar: '/avatar-3.png' },
        { name: 'Ryu', role: 'Graphic designer', avatar: '/avatar-2.png' },
        { name: 'The Net Ninja', role: 'Web daveloper', avatar: '/avatar-1.png' },
        { name: 'Gouken', role: 'Social media maverick', avatar: '/avatar-4.png' },
      ]
    }
  },
  name: "TeaM",
};
</script>
